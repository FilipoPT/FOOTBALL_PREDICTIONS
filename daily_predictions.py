...
def match_team_name(name, known_teams, cutoff=0.82):
    ...
    sem sufixos tipo SC/CP/FC), (4) fuzzy match como ultimo recurso, com
    cutoff alto (0.82) -- um cutoff baixo deixa passar falsos positivos
    perigosos (ex: 'Mallorca' acabou a apontar para 'Alcorcon' com cutoff
    0.55, so por coincidencia de letras -- isso e pior do que nao ter
    previsao nenhuma, porque parece confiante e nao e)."""
...
def _normalize_loose(name):
    ...
    muito entre fontes (CP, SC, FC, Clube, de, etc), tanto no meio/fim como
    no INICIO do nome (ex: 'SC Braga' -> 'braga'). So para o fallback de
    ...
    for token in [" cp", " sc", " fc", " clube", " de portugal", " clube de", "afc "]:
        n = n.replace(token, " ")
    # sufixos/prefixos genericos no INICIO do nome (ex: "SC Braga", "CD Castellon")
    prefixes = ["sc ", "cp ", "fc ", "cd ", "cf ", "ud ", "sd ", "afc ", "rc "]
    for p in prefixes:
        if n.startswith(p):
            n = n[len(p):]
            break
...
TEAM_ALIASES = {
    ...
    # Equipas SEM historico em nenhuma liga do dataset (recem-subidas/descidas,
    # sem dados ainda). Adicionar aqui nao gera previsao -- so faz a mensagem
    # de erro dizer "reconhecida, sem historico" em vez de "nome desconhecido",
    # o que e mais informativo. Corrige/atualiza o alvo assim que tiveres
